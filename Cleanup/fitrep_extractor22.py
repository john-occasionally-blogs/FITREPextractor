...
class FITREPExtractor:
    def __init__(self):
        ...
        self.valid_grades = [
            'SGT', 'SSGT', 'GYSGT', 'MGYSGT', '1STSGT', 'SGTMAJ',
    ...
    def extract_from_pdf(self, pdf_path):
        ...
        try:
            ...
            if len(doc) > 0:
                ...
                # Extract Grade using same approach as OCC (label with value below)
                grade_value = None
                grade_labels = ["Grade", "GRADE", "grade"]
                grade_indices = self.find_label_indices(ocr_data, grade_labels)
                
                # Only look in top third of page for Grade
                top_third_height = img.height // 3
                grade_indices = [i for i in grade_indices if ocr_data["top"][i] < top_third_height]
                
                # Sort by position (top to bottom, left to right) and take the FIRST one
                if grade_indices:
                    grade_indices.sort(key=lambda i: (ocr_data["top"][i], ocr_data["left"][i]))
                    
                    # Use only the first Grade label found
                    idx = grade_indices[0]
                    row_y = ocr_data["top"][idx]
                    
                    # Collect tokens that could be part of the grade
                    potential_tokens = []
                    
                    # Look at tokens below or to the right of Grade label
                    for k in range(idx + 1, min(idx + 25, len(ocr_data["text"]))):
                        # Allow more vertical distance for "below" since Grade might be on its own line
                        if ocr_data["top"][k] < row_y - 10:  # Skip if above
                            continue
                        if ocr_data["top"][k] > row_y + 80:  # Too far below
                            break
                            
                        tok = self.normalize_token(ocr_data["text"][k])
                        if tok:  # Collect all non-empty tokens
                            potential_tokens.append(tok)
                            ...
                            if corrected in self.valid_grades:
                                grade_value = corrected
                                break
                    
                    # If not found, try concatenating adjacent tokens (for split grades like "LT COL")
                    if not grade_value and len(potential_tokens) >= 2:
                        for i in range(len(potential_tokens) - 1):
                            # Try combining two adjacent tokens
                            combined = potential_tokens[i] + potential_tokens[i + 1]
                            if combined in self.valid_grades:
                                grade_value = combined
                                break
                            # Try with space removed in case OCR split it
                            combined_no_space = (potential_tokens[i] + potential_tokens[i + 1]).replace(" ", "")
                            if combined_no_space in self.valid_grades:
                                grade_value = combined_no_space
                                break
                            # Special handling for ranks that might be split
                            if potential_tokens[i] == "LT" and potential_tokens[i + 1] == "COL":
                                grade_value = "LTCOL"
                                break
                            if potential_tokens[i] == "MAJ" and potential_tokens[i + 1] == "GEN":
                                grade_value = "MAJGEN"
                                break
                            if potential_tokens[i] == "LT" and potential_tokens[i + 1] == "GEN":
                                grade_value = "LTGEN"
                                break
                            if potential_tokens[i] in ["1ST", "IST"] and potential_tokens[i + 1] in ["LT", "SGT"]:
                                if potential_tokens[i + 1] == "LT":
                                    grade_value = "1STLT"
                                else:
                                    grade_value = "1STSGT"
                                break
                            if potential_tokens[i] in ["2ND", "2D"] and potential_tokens[i + 1] == "LT":
                                grade_value = "2NDLT"
                                break
                
                # Fallback: scan top third for valid grade
                if not grade_value:
                    for i, text in enumerate(ocr_data["text"]):
                        if not text or ocr_data["top"][i] > top_third_height:
                            continue
