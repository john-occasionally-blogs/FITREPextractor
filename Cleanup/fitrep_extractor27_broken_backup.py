    ...
    def extract_from_pdf(self, pdf_path):
        ...
        try:
            ...
            if len(doc) > 0:
                ...
                # Extract To date - using same approach as Grade (label with value below)
                to_value = None
                to_labels = ["To", "TO", "To:", "TO:"]
                to_indices = self.find_label_indices(ocr_data, to_labels)
                
                # Only look in top third of page 
                top_third_height = img.height // 3
                ...
                if to_indices:
                    # Sort by position and look for one that's likely after "From"
                    to_indices.sort(key=lambda i: (ocr_data["top"][i], ocr_data["left"][i]))
                    
                    # Try to find the "To" that comes after "From"
                    best_to_idx = None
                    for idx in to_indices:
                        # Check if there's a "From" before this "To" on the same row-ish
                        row_y = ocr_data["top"][idx]
                        found_from_before = False
                        
                        # Look backwards for "From"
                        for j in range(max(0, idx - 10), idx):
                            if abs(ocr_data["top"][j] - row_y) < 30:  # Same row
                                tok = self.normalize_token(ocr_data["text"][j])
                                if tok in ["FROM", "FRON"]:  # FRON for OCR errors
                    ...
                    # If no "To" with "From" before it, just use the first one
                    if best_to_idx is None:
                        best_to_idx = to_indices[0]
                        print("    Using first 'To' at index {0}".format(best_to_idx))
                    
                    # Now look for 8-digit date below or near this "To"
                    row_y = ocr_data["top"][best_to_idx]
                    
                    # Look at tokens below or to the right
                    for k in range(best_to_idx + 1, min(best_to_idx + 20, len(ocr_data["text"]))):
                        # Allow date to be below (up to 60 pixels) or on same line
                        if ocr_data["top"][k] < row_y - 10:  # Skip if above
                            continue
                        if ocr_data["top"][k] > row_y + 60:  # Too far below
                            break
                        
                        tok = ocr_data["text"][k].strip()
                        print("      Checking token: '{0}'".format(tok))
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(1), tok):
                        ...
                        # Sometimes OCR adds spaces in the date
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        traceback.print_exc()
        sys.exit(1), tok_no_space):
                            to_value = tok_no_space
                            print("    Found date (after removing spaces): {0}".format(tok_no_space))
                            break
                
                # Fallback: if still no date, try the original method as last resort
                if not to_value:
                    print("    Fallback: looking for second 8-digit date in text")
                    # Use regular string, not raw string
                    date_pattern = '\\d{8}'
                    two_dates = re.findall(date_pattern, text1[:len(text1)//3])  # Only top third
                    if len(two_dates) >= 2:
                        to_value = two_dates[1]
                        print("    Found as second date: {0}".format(to_value))
